...
int main()
{
	char nMahasiswa[50][100], jMahasiswa[50][100], kMahasiswa[50][100];
	...
	while(entry == 'Y' || entry == 'y')
	{
		...
		if( strcmp(kMahasiswa[i],"SI")==0){
			strcpy(jMahasiswa[i],"Sistem Informasi");
			bayarAwal[i] = 2100000;
			bayarCicilan[i] = 7;
			besaran[i] = 300000;
			uangKuliah[i] = bayarAwal[i] + (bayarCicilan[i] * besaran[i]);
		}else if(strcmp(kMahasiswa[i], "TI")==0){
			strcpy(jMahasiswa[i], "Teknik Informatika");
			bayarAwal[i] = 2500000;
			bayarCicilan[i] = 7;
			besaran[i] = 300000;
			uangKuliah[i] = bayarAwal[i] + (bayarCicilan[i] * besaran[i]);
		}else if(strcmp(kMahasiswa[i],"KA")==0){
			strcpy(jMahasiswa[i],"Komputer Akuntansi");
			bayarAwal[i] = 1750000;
			bayarCicilan[i] = 6;
			besaran[i] = 200000;
			uangKuliah[i] = bayarAwal[i] + (bayarCicilan[i] * besaran[i]);
		}else if(strcmp(kMahasiswa[i],"MI")==0){
			strcpy(jMahasiswa[i],"Manajemen Informatika");
			bayarAwal[i] = 1500000;
			bayarCicilan[i] = 6;
			besaran[i] = 250000;
			uangKuliah[i] = bayarAwal[i] + (bayarCicilan[i] * besaran[i]);
